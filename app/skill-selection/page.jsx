...
const SkillSelection = () => {
  ...
  const toggleSkill = (skill) => {
    setFormData((prev) => {
      const isAlreadySelected = prev.skills.some((s) => s.id === skill.id);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (formData.skills.length === 0 || !formData.experience) return;
    setIsLoading(true);

    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("authToken") : null;
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/sessions`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          job_role: formData.skills.map((s) => s.name).join(", "),
                  ...
                  {formData.skills.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {formData.skills.map((skill) => (
                        <span
                          key={skill.id}
                          className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white/20 text-white backdrop-blur-sm border border-white/10"
                        >
                          {skill.name}
                          <button
                            type="button"
                  ...
                  <form onSubmit={handleSubmit}>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        {experienceOptions.map((option) => (
                          <button
                            ...
                            onClick={() =>
                              handleInputChange({
                                target: { name: "experience", value: option.value },
                              })
                            }
                    ...
                    <div className="pt-4">
                      <button
                        ...
                        className="w-full bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 text-white py-5 rounded-2xl font-bold text-lg transition-all duration-300 hover:shadow-2xl transform hover:-translate-y-2 hover:scale-[1.02] flex items-center justify-center gap-4 relative overflow-hidden group disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                      >
                        {/* Button glow effect */}
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-400 via-purple-400 to-indigo-400 blur-lg opacity-0 group-hover:opacity-50 transition-opacity duration-300 pointer-events-none"></div>
                        ...
                        {isLoading ? (
                          ...
                          <>
                            ...
                            <span className="relative z-10">Start Skill Interview</span>
