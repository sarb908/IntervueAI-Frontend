...
const InterviewPage = () => {
  ...
  const submitAnswer = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/sessions/${sessionData.id}/answers`,
        {
          method: "POST",
          headers: {
  ...
  const transcribeAudio = async () => {
    try {
      ...
      // Send to backend
      const token = typeof window !== "undefined" ? localStorage.getItem("authToken") : null;
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/sessions/stt`, {
        method: 'POST',
        headers: {
          ...(token ? { "Authorization": `Bearer ${token}` } : {})
        },
  ...
  useEffect(() => {
    ...
    const speakQuestion = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/sessions/tts`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(token ? { "Authorization": `Bearer ${token}` } : {}),
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 border border-gray-200 space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button
                  ...
                  className={`flex items-center ml-0 gap-2 px-4 py-2 rounded-lg font-medium ${isRecording
